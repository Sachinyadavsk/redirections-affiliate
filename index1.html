...
<body>
    ...
    <main class="container">
        ...
        <div class="tester-box">
            ...
            <div class="result-options">
                Show result as:
                <label>
                    <input type="radio" name="resultType" value="redirect" checked>Redirections
                </label>
            </div>
        </div>
    ...
    <script>
        ...
        function toggleAdvanced() {
            const box = document.getElementById("advancedContent");
        ...
        function updateCarriers(country) {
            ...
            if (document.getElementById("advancedNetType").value === "3g") {
        ...
        function getFormData() {
            ...
            const advancedDevice = document.getElementById("advancedDev").value;
            const city = document.getElementById("advancedCity").value;
            const connection = document.getElementById("advancedNetType").value;
            const carrier = document.getElementById("advancedASN").value;
            const resultType = document.querySelector('input[name="resultType"]:checked')?.value || "redirect";
        ...
        function validateForm(formData) {
            ...
            if (formData.connection === "3g" && !formData.carrier) {
                ...
                document.getElementById("advancedASN").focus();
                return false;
            }
        ...
        async function runTest() {
            ...
            try {
                ...
                //    Current third-party API
                const apiUrl = "https://www.redirectcheck.org/api/check" +
                    "?url=" +
                    encodeURIComponent(requestData.url);
                const response = await fetch(apiUrl);
                if (!response.ok) {
                    throw new Error("Redirect API returned HTTP " + response.status);
                }
                const data = await response.json();
                if (data.error) {
                    throw new Error(data.error_message || "Unable to check this URL.");
                }
        ...
        function displayResults(data, requestData) {
            ...
            if (visibleRedirects.length === 0) {
                ...
                //    REDIRECTS
                const directUrl = data.url || requestData.mainInput || "-";
                 list.innerHTML += `<div class="url-line">${makeUrlLink(directUrl)}</div>`;

                visibleRedirects.forEach((redirect, index) => {
                    const from = redirect.from || redirect.url || "-";
